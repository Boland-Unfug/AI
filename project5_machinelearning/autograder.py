    ...
    def begin_q(self, q):
        ...
        for prereq in sorted(self.prereqs[q]):
            if self.points[prereq] < self.maxes[prereq]:
                print(
                    """*** NOTE: Make sure to complete Question {} before working on Question {},
...
""".format(
    ...
    def finalize(self):
        ...
        print(
            "Total: %d/%d"
            % (sum(self.points.values()), sum([self.maxes[q] for q in self.questions]))
        )
...
def verify_node(node, expected_type, expected_shape, method_name):
    if expected_type == "parameter":
        ...
        assert isinstance(
            node, (nn.SquareLoss, nn.SoftmaxLoss)
        ), "{} should return a loss node, instead got type {!r}".format(
            method_name, type(node).__name__
        )
    elif expected_type == "node":
        assert node is not None, "{} should return a node object, not None".format(
            method_name
        )
        assert isinstance(
            node, nn.Node
    ...
    if expected_type != "loss":
        assert all(
            ...
            method_name,
            nn.format_shape(expected_shape),
...
def check_perceptron(tracker):
    ...
    for dimensions in range(1, 10):
        ...
        verify_node(
            p_weights, "parameter", (1, dimensions), "PerceptronModel.get_weights()"
        )
    ...
    for multiplier in (-5, -2, 2, 5):
        ...
        sanity_dataset = backend.Dataset(
            x=np.tile(point, (500, 1)), y=np.ones((500, 1)) * -1.0
        )
    ...
    accuracy = np.mean(
        np.where(np.dot(dataset.x, model.get_weights().data.T) >= 0.0, 1.0, -1.0)
        == dataset.y
    )
    if accuracy < 1.0:
        print(
            "The weights learned by your perceptron correctly classified {:.2%} of training examples".format(
                accuracy
            )
        )
        print(
...
def check_regression(tracker):
    ...
    for batch_size in (1, 2, 4):
        ...
        assert (
            inp_x in trace
        ), "Node returned from RegressionModel.run() does not depend on the provided input (x)"
        ...
        for node in trace:
            assert (
                not isinstance(node, nn.Parameter) or node in detected_parameters
    ...
    for batch_size in (1, 2, 4):
        ...
        assert (
            inp_x in trace
        ), "Node returned from RegressionModel.get_loss() does not depend on the provided input (x)"
        assert (
            inp_y in trace
        ), "Node returned from RegressionModel.get_loss() does not depend on the provided labels (y)"
    ...
    verify_node(
        train_predicted, "node", (dataset.x.shape[0], 1), "RegressionModel.run()"
    )
    ...
    assert np.isclose(train_loss, sanity_loss), (
        ...
        "based on the output of RegressionModel.run()".format(train_loss, sanity_loss)
    )
    ...
    if train_loss <= loss_threshold:
        ...
        print(
            "Your final loss ({:f}) must be no more than {:.4f} to receive full points for this question".format(
...
def check_digit_classification(tracker):
    ...
    for batch_size in (1, 2, 4):
        ...
        output_node = model.run(inp_x)
        verify_node(
            output_node, "node", (batch_size, 10), "DigitClassificationModel.run()"
        )
        trace = trace_node(output_node)
        assert (
            inp_x in trace
        ), "Node returned from DigitClassificationModel.run() does not depend on the provided input (x)"
    ...
    for batch_size in (1, 2, 4):
        ...
        assert (
            inp_x in trace
        ), "Node returned from DigitClassificationModel.get_loss() does not depend on the provided input (x)"
        assert (
            inp_y in trace
        ), "Node returned from DigitClassificationModel.get_loss() does not depend on the provided labels (y)"
...
def check_lang_id(tracker):
    ...
    for batch_size, word_length in ((1, 1), (2, 1), (2, 6), (4, 8)):
        ...
        inp_xs, inp_y = dataset._encode(
            dataset.dev_x[start:end], dataset.dev_y[start:end]
        )
        ...
        verify_node(
            output_node,
            "node",
            (batch_size, len(dataset.language_names)),
            "LanguageIDModel.run()",
        )
        trace = trace_node(output_node)
        for inp_x in inp_xs:
            assert (
                inp_x in trace
            ), "Node returned from LanguageIDModel.run() does not depend on all of the provided inputs (xs)"
    ...
    for batch_size, word_length in ((1, 1), (2, 1), (2, 6), (4, 8)):
        ...
        assert (
            inp_y in trace
        ), "Node returned from LanguageIDModel.get_loss() does not depend on the provided labels (y)"
    ...
    # print("num chars", dataset.model.num_chars)
    # print("Running training loop", dataset.bucket_weights.shape)
    # print("Running training loop", dataset.dev_buckets.shape)
    # print("Running training loop", dataset.test_buckets.shape)
    # print("Running training loop", dataset.train_buckets.shape)
    # print("Running training loop", dataset.dev_x.shape)
    # print("Running training loop", dataset.dev_y.shape)
    # print("Running training loop", dataset.test_x.shape)
    # print("Running training loop", dataset.test_y.shape)
    print("Running training loop", dataset.train_x.shape)
    print("Running training loop", dataset.train_x)
    # print("Running training loop", dataset.train_y.shape)
    # print("Running training loop", vars(dataset.model))
    # print("Running training loop", vars(dataset))

    model.train(dataset)
    # print("Running prediction on dev set", dataset)
