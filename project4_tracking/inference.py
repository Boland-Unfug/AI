...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in variableDomainsDict[PAC]:
        for j in variableDomainsDict[GHOST0]:
            for k in range(MAX_NOISE + 1):
                if(manhattanDistance(i, j) + k) not in variableDomainsDict[OBS0]:
                    variableDomainsDict[OBS0].append(manhattanDistance(i, j) + k)
    ...
    for i in variableDomainsDict[PAC]:
        for j in variableDomainsDict[GHOST1]:
            for k in range(MAX_NOISE + 1):
                if(manhattanDistance(i, j) + k) not in variableDomainsDict[OBS1]:
                    variableDomainsDict[OBS1].append(manhattanDistance(i, j) + k)
...
def inferenceByEnumeration(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict):
    ...
    for eliminationVariable in bayesNet.variablesSet() - set(queryVariables) - set(evidenceDict.keys()):
        fullJoint = eliminate(fullJoint, eliminationVariable)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        queryConditionedOnEvidence = normalize(joinFactors(currentFactorsList))
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        # If the ghost is in jail, then the noisy distance should be None
        if ghostPosition == jailPosition:
            return 1 if noisyDistance is None else 0
        
        # If the noisy distance is None, then the ghost is in jail
        if noisyDistance is None:
            return 0
        
        # Otherwise, calculate the true distance and return the probability
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            #print(ghostPosition)
            #print(self.beliefs[ghostPosition])
            #print(self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition))
            # I can't multiply a float by a DiscreteDistribution, so I have to do this
            newBeliefs[ghostPosition] = self.beliefs[ghostPosition] * self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        # Calculate the new beliefs, why is the number slightly off?
        newBeliefs = DiscreteDistribution()
        for oldGhostPosition in self.allPositions:
            newGhostPositionDist = self.getPositionDistribution(gameState, oldGhostPosition)
            for newGhostPosition in newGhostPositionDist:
                newBeliefs[newGhostPosition] += newGhostPositionDist[newGhostPosition] * self.beliefs[oldGhostPosition]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        jailPosition = self.getJailPosition()
        for particle in self.particles:
            newBeliefs[particle] += self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
        ...
        for i in range(self.numParticles):
            newParticles.append(self.beliefs.sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghost in range(self.numGhosts):
            ...
            for particle in self.particles:
                newBeliefs[particle] += self.getObservationProb(observation[ghost], pacmanPosition, particle[ghost], jailPosition)
